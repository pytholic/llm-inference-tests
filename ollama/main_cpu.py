...
def generate_response(
    ...
    metrics = {}  # Initialize metrics here
...
if __name__ == "__main__":
    ...
    # Perform the warmup run
    print("\n***Warmup run***\n")
    warmup_metrics = generate_response(prompt, options=options)
    first_prompt_eval_duration = warmup_metrics.get("prompt_eval_duration", 0)
    gc.collect()

    # Generate response
    num_runs = 3
    peak_memory_usage = 0  # Initialize peak memory usage
    ...
    # Run the remaining iterations and collect metrics
    for i in range(num_runs):
        print(f"\n***Response {i+1} from model***\n")
        metrics = generate_response(prompt, options=options)

        avg_metrics["total_duration"] += metrics.get("total_duration", 0)
        avg_metrics["time_to_first_token"] += metrics.get("prompt_eval_duration", 0)
        avg_metrics["eval_count"] += metrics.get("eval_count", 0)
        avg_metrics["eval_duration"] += metrics.get("eval_duration", 0)

        gc.collect()

    # Calculate averages
    if num_runs > 0:
        avg_metrics["total_duration"] /= num_runs
        avg_metrics["time_to_first_token"] /= num_runs
        avg_metrics["time_per_output_token"] = (
            avg_metrics["eval_duration"] / avg_metrics["eval_count"]
            if avg_metrics["eval_count"] > 0
            else 0
        )
        avg_metrics["tokens_per_second"] = (
            avg_metrics["eval_count"] / avg_metrics["eval_duration"]
            if avg_metrics["eval_duration"] > 0
            else 0
        )
        avg_metrics["eval_count"] /= num_runs
        avg_metrics["eval_duration"] /= num_runs

    print("\n\nAverage Metrics:")
    print(f"First Prompt Evaluation Time: {first_prompt_eval_duration:.3f}s")
    print(f"Total Duration: {avg_metrics['total_duration']:.3f}s")
    print(f"Time to First Token: {avg_metrics['time_to_first_token']:.3f}s")
    print(f"Time Per Output Token: {avg_metrics['time_per_output_token']:.3f}s")
    print(f"Tokens Per Second: {avg_metrics['tokens_per_second']:.2f}")
    print(f"Eval Count: {avg_metrics['eval_count']}")
    print(f"Eval Duration: {avg_metrics['eval_duration']:.3f}s")
